...
def preprocess(use_local: bool, use_sampled_data: bool):
    ...
    # print("test print")
    train = pd.read_csv(
        f"{data_loc_prefix}train{use_sampled_data_str}.csv", dtype=DTYPES
    )
    ...
    for usecol in train.columns.tolist()[1:-1]:
        ...
        agg = agg[
            (agg["Train"] / agg["Total"] > 0.2) & (agg["Train"] / agg["Total"] < 0.8)
        ]
        agg[usecol + "Copy"] = agg[usecol]

        train[usecol] = (
            pd.merge(
                train[[usecol]], agg[[usecol, usecol + "Copy"]], on=usecol, how="left"
            )[usecol + "Copy"]
            .replace(np.nan, 0)
            .astype("int")
            .astype("category")
        ...
        test[usecol] = (
            pd.merge(
                test[[usecol]], agg[[usecol, usecol + "Copy"]], on=usecol, how="left"
            )[usecol + "Copy"]
            .replace(np.nan, 0)
            .astype("int")
            .astype("category")
    ...
    test_ids = test.index
    train_ids.to_series().to_pickle(
        f"{data_loc_prefix}_train_index{use_sampled_data_str}.pkl"
    )
    test_ids.to_series().to_pickle(
        f"{data_loc_prefix}_test_index{use_sampled_data_str}.pkl"
    )
    ...
    train = vstack(
        [
            ohe.transform(train[i * m : (i + 1) * m])
            for i in range(train.shape[0] // m + 1)
        ]
    )
    test = vstack(
        [
            ohe.transform(test[i * m : (i + 1) * m])
            for i in range(test.shape[0] // m + 1)
        ]
    )
    save_npz(
        f"{data_loc_prefix}_train{use_sampled_data_str}.npz", train, compressed=True
    )
